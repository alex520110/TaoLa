...
(function($) {
	$.fn.TaoLaDistrict = function(options) {
		var defaults = {
			// 设置id
			id : $(this).attr("id"),
			// 指定绑定源
		...
		this.each(function() {
			var _this = $(this);
			var district=_this.siblings('.TaoLa-district');
			...
			var thisId = 'TaoLaDistrict-' + params.id;
			...
			var TaoLaDistrict = '<div class="TaoLa-district" id="'+thisId+'"><div class="district-hd"><span>请选择省</span></div>'+provinceStr+'<ul class="district-ul city-ul cl"></ul><ul class="district-ul county-ul cl"></ul></div>';
			...
			if(params.renderTo.find("#"+thisId).length>0){
				...
				params.renderTo.append(TaoLaDistrict);
				_this.addClass('active');
			}
			...
			params.renderTo.on('click','.city-ul a',function() {
				...
				if(parent.county!=null){
					...
					$(this).parents('.TaoLa-district').hide();
					params.closeFn();
				}
