...
(function ($) {
    ...
    function request(target, param) {
        var opts = $.data(target, "TaoLaDatagrid").options;
        if (param) {
    ...
    function getSelectRows(target) {
        var opts = $.data(target, "TaoLaDatagrid").options;
        var data = $.data(target, "TaoLaDatagrid").data;
        var tBody = $(target).find("tbody");
        if (opts.idField) {
            return $.data(target, "TaoLaDatagrid").selectedRows;
        } else {
            var rowAry = [];
            $("tr.TaoLaDatagrid-row-selected", tBody).each(function () {
                var row_index = parseInt($(this).attr("TaoLaDatagrid-row-index"));
    ...
    function selectAll(target) {
        var opts = $.data(target, "TaoLaDatagrid").options;
        var data = $.data(target, "TaoLaDatagrid").data;
        var selectRows = $.data(target, "TaoLaDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        var rows = data.rows;

        $("tr", tBody).addClass("TaoLaDatagrid-row-selected");
        $("div.TaoLaDatagrid-cell-check input[type=checkbox]", tBody)
            .not("div.TaoLaDatagrid-cell-check input[disabled]")
            .prop("checked", true);
    ...
    function clearSelectRows(target) {
        ...
        var selectRows = $.data(target, "TaoLaDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        $("tr", tBody).removeClass("TaoLaDatagrid-row-selected");
        $("div.TaoLaDatagrid-cell-check input[type=checkbox]", tBody).prop("checked", false);
    ...
    function selectRow(target, index) {
        ...
        var tr = $("tr[TaoLaDatagrid-row-index=" + index + "]", tBody);
        if (!tr.hasClass("TaoLaDatagrid-row-selected")) {
            tr.addClass("TaoLaDatagrid-row-selected");
            var ck = $("div.TaoLaDatagrid-cell-check input[type=checkbox]", tr);
    ...
    function unSelectRow(target, index) {
        ...
        var tBody = $(target).find("tbody");
        var tr = $("tr[TaoLaDatagrid-row-index=" + index + "]", tBody);
        var ck = $("tr[TaoLaDatagrid-row-index=" + index + "] div.TaoLaDatagrid-cell-check input[type=checkbox]", tBody);
        tr.removeClass("TaoLaDatagrid-row-selected");
    ...
    function bindCellsEvents(target) {
        var opts = $.data(target, "TaoLaDatagrid").options;
        var total = $.data(target, "TaoLaDatagrid").data.total;
        var before = $("a.beforePageBtn", $(target).next());
        var after = $("a.afterPageBtn", $(target).next());
        var pageCount = parseInt(total / opts.pageSize) + (total % opts.pageSize == 0 ? 0 : 1);


        //翻页跳转弹框
        $('#jump-to,.tipic-dialog').click(function (e) {
            e ? e.stopPropagation() : event.cancelBubble = true;
        });

        $('#jump-to').click(function () {
            $('.tipic-dialog').fadeToggle(200).find('input').first().focus();
        });
        ...
        $("input.check-all", $(target).next()).unbind("click").bind("click", function () {
        ...
        $("div.TaoLaDatagrid-cell-check input[type=checkbox]").unbind("click").bind("click", function (e) {
            var findIndex = $(this).parent().parent().parent().attr("TaoLaDatagrid-row-index");
            if (opts.singleSelect) {
                clearSelections(target);
    ...
    function loadData(target, _data) {
        ...
        $.data(target, "TaoLaDatagrid").data = _data;
    ...
    //插件构造函数
    $.fn.TaoLaDatagrid = function (options, param) {
        if (typeof options == "string") {
            return $.fn.TaoLaDatagrid.methods[options](this, param);
        }

        options = options || {};
        return this.each(function () {
            var state = $.data(this, "TaoLaDatagrid");
            var opts;
            if (state) {
                opts = $.extend(state.options, options);
                state.options = opts;
            } else {
                opts = $.extend({}, $.fn.TaoLaDatagrid.defaults, options);
                //console.log(opts);
                $.data(this, "TaoLaDatagrid", {
                    options: opts,
                    selectedRows: [],
                    data: {
                        total: 0,
    ...
    //插件对外公开的所有方法集合
    $.fn.TaoLaDatagrid.methods = {
        options: function (jq) {
            var opts = $.data(jq[0], "TaoLaDatagrid").options;
            return opts;
        },
        ...
        reload: function (jq, param) {
            return jq.each(function () {
                ...
                $.data(this, "TaoLaDatagrid").options.pageNumber = param.pageNumber;
                request(this, param);
            });
        },
        //显示正在加载提示信息
        loading: function (jq) {
            return jq.each(function () {
                var opts = $.data(this, "TaoLaDatagrid").options;
    ...
    var renderView = {
        render: function (target, container) {
            var opts = $.data(target, "TaoLaDatagrid").options;
            var rows = $.data(target, "TaoLaDatagrid").data.rows;
            var closFileds = $(target).TaoLaDatagrid("getColumnFields");

            if (rows == null || rows.length <= 0) {
                $("#loadAjax").remove();
                var html = ["<tbody><tr><td style='text-align:center;' colspan=" + opts.columns[0].length + "><h2 style='font-size: 18px;padding-bottom: 10px;'>" + opts.NoDataMsg + "</h2></td></tr></tbody>"];
                ...
                var htmlAry = ["<tbody>"];
                for (var i = 0; i < rows.length; i++) {
                    var cls = (i % 2 && opts.striped) ? "class=\"TaoLaDatagrid-row-alt\"" : "";
                    var rowStyle = opts.rowStyler ? opts.rowStyler.call(target, i, rows[i]) : "";
                    var strStyle = rowStyle ? "style=\"" + rowStyle + "\"" : "";
                    htmlAry.push("<tr TaoLaDatagrid-row-index=\"" + i + "\" " + cls + " " + strStyle + ">");
        ...
        renderHead: function (target, container) {
            ...
            for (var i = 0; i < closFileds.length; i++) {
                var content = "";
                var className = "";
                var findFiled = closFileds[i];
                var col = $(target).TaoLaDatagrid("getColumnOption", findFiled);
                ...
                htmlAry.push("<th " + className + style + ">");
        ...
        renderRow: function (target, fileds, rowIndex, rowData) {
            ...
            for (var i = 0; i < fileds.length; i++) {
                ...
                if (col) {
                    ...
                    if (col.checkbox) {
                        cc.push(" class=\"TaoLaDatagrid-cell-check ");
                    } else {
                        cc.push(" class=\"TaoLaDatagrid-cell ");
                    }
                    cc.push("\">");
                    if (col.checkbox) {
                        if (col.formatter) {
