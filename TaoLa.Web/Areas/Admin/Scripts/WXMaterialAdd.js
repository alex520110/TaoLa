...
$(document).ready(function () {
    ...
    $(".item-wrapper03").click(function () {
        ...
        $(".glyphicon-arrow-down").click(function () {
            $(this).parents(".item ").before($(this).next(".item"));
        });
        $(".glyphicon-arrow-up").click(function () {
            $(this).parents(".item ").after($(this).prev(".item"));
        });
        navCnt = navCnt + 1;
    ...
    $('span[name=btnSave]').click(SaveMaterial);
...
{//clone右则素材输入框
    ...
    $('span[name=btnSave]', content).unbind('click').click(SaveMaterial);
...
function SaveMaterial()
{
    ...
    $('.item',$('.WX-edit')).each(function (idx, el) {
        ...
        if ($content.length>0)
        {
            ...
            if ($('input[name=setUrl]', $content).get(0).checked) {
                material.push('content_source_url:"' + $('input[name=content_source_url]', $content)+ '"');
            } else {
    ...
    if (!isCheck)
    {
        $.dialog.alert('标题、作者、内容、封面图片不能为空');
        return;
    }
