...
namespace TaoLa.Core
{
	public class StringHelper
	{
		public static int GetStringLength(string s)
		{
			...
			{
				result = System.Text.Encoding.Default.GetBytes(s).Length;
			}
			else
			{
				result = 0;
		...
		public static string TrimStart(string sourceStr, string trimStr, bool ignoreCase)
		{
			...
			else if (string.IsNullOrEmpty(trimStr) || !sourceStr.StartsWith(trimStr, ignoreCase, System.Globalization.CultureInfo.CurrentCulture))
			{
				result = sourceStr;
			}
			else
		...
		public static string TrimEnd(string sourceStr, string trimStr, bool ignoreCase)
		{
			...
			else if (string.IsNullOrEmpty(trimStr) || !sourceStr.EndsWith(trimStr, ignoreCase, System.Globalization.CultureInfo.CurrentCulture))
			{
				result = sourceStr;
			}
			else
